...
pub(crate) fn generate_abs_enums(
    ...
    for field in struct_data.fields.iter() {
        ...
        if let Some(child_struct) = data_structure.struct_map.get(&field.ty_string) {
            let child_namespace = child_struct.type_names.namespace.clone();
            let child_struct_abs_key = child_struct.type_names.abs_key_enum.clone();
            let child_struct_abs_field = child_struct.type_names.abs_field_enum.clone();

            abs_key_variants.push(quote! {
                #field_name(super::#child_namespace::#child_struct_abs_key)
            });

            abs_field_variants.push(quote! {
                #field_name(super::#child_namespace::#child_struct_abs_field)
            });
        } else {
