...
use crate::{
    casing::to_camel_case, data_structure::{self, absolute_path::{AbsolutePath, AbsolutePathField}, conditional_paths::{AllFolderFields, FolderFocusPath, FolderFocusPathVector}}, DataStructure
};
...
fn generate_get_set(_crate_path: &TokenStream2, data_structure: &DataStructure) -> TokenStream2 {
    ...
    for (folder_name, folder_info) in data_structure.all_folder_fields.iter() {
        if folder_info.fields.len() <= 1 {
            continue;
        }
        ...
        let get_focus_function_name = format_ident!("get_{}_path", folder_focus_variable);

        let folder_focus_path_vector = data_structure.folder_focus_path_map.get(folder_name).unwrap();

        let focus_path_stream = recurse_conditional(
    ...
    if let Some(field) = abs_path.pop() {
        match field {
            ...
            AbsolutePathField::Struct((field_name, struct_data)) => {
                let field_name = format_ident!("{}", to_camel_case(&field_name));
                let struct_namespace = struct_data.type_names.namespace.clone();
                ...
                let key_stream = quote! {
                    #field_name(super::#struct_namespace::#struct_key_enum::#child_keys)
                };

                let field_stream = quote! {
                    #field_name(super::#struct_namespace::#struct_field_enum::#child_fields)
                };
...
fn generate_trait_impl(crate_path: &TokenStream2, data_structure: &DataStructure) -> TokenStream2 {
    ...
    quote! {
        #[automatically_derived]
        impl #crate_path::FocusHandler<super::#root_struct_description> for #focus_handler_name {
