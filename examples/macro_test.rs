...
use database::{
    AbsFieldConstraints, AbsKeyConstraints, DataFieldAccessor,
    DatabaseDescription, FlatFieldConstraints, FlatKeyConstraints,
    ToKey, VariantCount, Folder,
    DatabaseError, DynamicKeySet, ToFromUsize, FolderHandler, ToFull, FocusHandler
};
...
impl DataFieldAccessor<root::AbsKey, root::AbsField> for Root {
    fn get(&self, _key: root::AbsKey) -> root::AbsField {
        todo!()
    }

    fn set(&mut self, _field: root::AbsField) {
        todo!()
    }
}

struct RootDescription {}
impl DatabaseDescription for RootDescription {
    type AbsKey = root::AbsKey;
    type AbsField = root::AbsField;
    type FlatKey = root::Key;
    type FlatField = root::Field;
    type Data = Root;
}
